...
const itemList: any[] = [
  ...
  {
    id: '2', title: '信托计划类型', code: 'trustPlanType', itemFlag: '', child: [
      {cId: '21', name: '单一', type: 'button', col: 'col-xs-1', val: '1'},
      {cId: '22', name: '集合', type: 'button', col: ' col-xs-1', val: '2'}]
  },
  {
    id: '3', title: '产品期限', style: 'special', itemClick: '35', itemFlag: '', itemHiddenFlag: '', child: [
      ...
      {cId: '37', name: '请输入', type: 'input', col: 'col-sm-4', itemHidden: true, code: 'dateStart', val: ''},
      {cId: '38', name: '-', type: 'span', itemHidden: true},
      {cId: '39', name: '请输入', type: 'input', col: 'col-sm-4', itemHidden: true, code: 'dateEnd', val: ''},
  ...
  {
    id: '4', title: '运作时间', code: 'operationTimeStart',itemFlag: '', child: [
...
@Component({
  templateUrl: 'trustPlan.component.html',
  styleUrls: ['../../base/org.hidden.css']
})
export class TrustPlanComponent implements OnInit {

  itemList = itemList;
  productState; // 产品状态 上下架使用
  toggleFlag: Boolean; // 隐藏标志
  isLoading: Boolean = false; // 加载标志
  buttonPermission: String = 'products.trustPlan.shelve'; // 按钮权限
  private seletedTrustPlan: TrustPlan; // 选中的人员列表
  /** 加载信息 */
  ...
  ''
  private trustPlan: TrustPlan[]; // 线下资金列表
  private param = new Object; // 查询参数

  constructor(private trustPlanService: TrustPlanService,
  ...
  recordCheck(trustPlan: TrustPlan) {
    this.seletedTrustPlan = trustPlan;
    this.productState = trustPlan.state;
  }
  ...
  query() {
    this.isLoading = true;
    this.trustPlanService.page(this.pageParams.curPage - 1, this.pageParams.pageData, this.param)
      .then(result => {
  ...
  updateState(productState: string): void {
    const me = this;
    // 操作判断
    if (!me.seletedTrustPlan) {
      this.messageService.alert('', '请选择一条记录');
      return;
    }
    me.trustPlanService.updateState(me.seletedTrustPlan.id, productState)
      .then(result => {
